...
def load_test_data(test_file, labels):
    ...
    if 'Category' in df.columns:
        ...
        print ";".join(df[select[1]])
        print label_dict.keys()
        y_ = df[select[1]].apply(lambda x: label_dict[x.decode('utf-8')]).tolist()
...
def predict_unseen_data():
    ...
    with tf.Graph().as_default():
        ...
        with sess.as_default():
            ...
            print ",".join(columns)
            df.to_csv(predicted_dir + 'predictions_all.csv', index=False,encoding='utf-8', columns=columns, sep=',')
